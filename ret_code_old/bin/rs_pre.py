    ...
    def load_obs_csv(self, csv_file, date_fmt="%Y/%m/%d %H:%M", mission_lst=None, only_geom=False):
        ...
        if ncol==10:
            ...
            for i,act_date in enumerate(date_lst):
                ...
                #-- BRFs start at 0-indexed column 8
                for ib in xrange(13):
                    icol = ib+8
...
#                        self.schedule_dct['sim_typ'].append(1) #-- !!! set BIT1 !!!
        ...
        msg = "common schedule yields time-points with: S1A:{} S1B:{}".format(
            n_s1a, n_s1b)
    ...
    def setprior_generic_agriculture(self):
        ...
        #-- set uncertainty for LAI coefficient
    ...
    def setprior_unc_default(self):
        ...
        #-- we apply
        #   - relative unceratainties on the state variables
        #   - uncertainty is limited below by a floor value
        self.lai_coeff_unc = np.maximum(self.lai_coeff*self.lai_coeff_relunc, self.lai_coeff_uncfloor)
        self.prstate_unc = np.empty(self.prstate.shape, dtype=np.float64)
        self.prstate_unc[0,:] = np.maximum(self.prstate[0,:]*self.lai_relunc,   self.lai_uncfloor)
        self.prstate_unc[1,:] = np.maximum(self.prstate[1,:]*self.canht_relunc, self.canht_uncfloor)
        self.prstate_unc[2,:] = np.maximum(self.prstate[2,:]*self.sm_relunc,    self.sm_uncfloor)
...
def create_argument_parser(progname=None):
    ...
    def _add_common_options(aparser):
        aparser.add_argument( '--time_start',
                              help="""retrieval system will discard time-points before the date given here (fmt: YYYYMMDDTHH:MM). Note however that further temporal restriction may result from invoking options --target_select or --target_schedule.""" )
        aparser.add_argument( '--time_end',
                              help="""retrieval system will discard time-points after the date given here (fmt: YYYYMMDDTHH:MM). Note however that further temporal restriction may result from invoking options --target_select or --target_schedule.""" )
    ...
    xparser.add_argument( '--mission_lst',
                          ...
                          help="""from which missions satellite geometries will be used (default: %(default)s)""" )
