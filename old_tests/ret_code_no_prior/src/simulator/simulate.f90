...
subroutine simulate_s1(n, x, m, y)
  ...
  real(kind=8) :: xpr_s1(nparam_s1+npts_s1*nsc), sxpr_s1(nparam_s1+npts_s1*nsc)
  ...
  ! !-- get S1 related prior
  ! !   Note: we need the uncertainties for re-conversion to physical units
  ! call getprior_s1(nparam_s1+npts_s1*nsc, xpr_s1, sxpr_s1)

  !-- map S1 parameter into model input vector
  !   reconvert to physical units
  !   ATTENTION: S1 parameter is part of model input
  statev(1:nparam_s1) = x(1:nparam_s1) !*sxpr_s1(1:nparam_s1)

  !-- loop over all possible sim
  i0 = nparam_s1 + 1
  j0 = 1
...
subroutine simulate_s2(n, x, m, y)
  ...
  ! !-- get S1 related prior
  ! !   Note: we need the uncertainties for re-conversion to physical units
  ! call getprior_s2(npts_s2*nsc, xpr_s2, sxpr_s2)
...
subroutine simulate_s1s2(n, x, m, y)
  ...
  !-- S2 simulation
  call simulate_s2(n_s2, x_s2, m_s2, y(m_s1+1:m_s1+m_s2))
