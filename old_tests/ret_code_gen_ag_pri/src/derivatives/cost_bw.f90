...
! fresnel0 = abs( (1._8 - sqrt(e))/(1._8+sqrt(e)) )**2
  ...
  CALL CSQ2_BW0_FIX(hlp, hlp_bw, fresnel0_bw)
  temp0 = SQRT(e)
...
! h = real(rho_h)**2 + aimag(rho_h)**2
  CALL CSQ2_BW0_FIX(rho_h, rho_h_bw, h_bw)
  CALL CSQ2_BW0_FIX(rho_v, rho_v_bw, v_bw)
...
! local decls
  ...
  REAL(kind=8) :: s12_dot_bw
  INTRINSIC SUM
  REAL(kind=8) :: convolve_bw
  REAL(kind=8) :: convolve
  s2_sum = SUM(s2)
  s12_dot_bw = convolve_bw/s2_sum
  s1_bw = s1_bw + s2*s12_dot_bw
