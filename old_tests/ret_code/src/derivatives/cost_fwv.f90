...
!-- compute state model differences
  ...
  ELSE
    ...
    END DO
  END IF
  IF (ldebug) WRITE(*, '(a,e25.16)') ' DIAG::cost:cost_model=', &
&             costmodel
!-- compute prior diff
...
&                     nbdirs)
  ...
  ELSE
    ...
    END DO
  END IF
  IF (ldebug) WRITE(*, '(a,e25.16)') ' DIAG::cost:cost_prior=', &
&             costprior
  DO nd=1,nbdirs
...
! local decls
  ...
  REAL(kind=8), DIMENSION(nbdirsmax) :: s12_dot_fwv
  INTRINSIC SUM
  ...
  DO nd=1,nbdirs
    ...
    s12_dot_fwv(nd) = SUM(arg1_fwv(nd, :))
    convolve_fwv(nd) = s12_dot_fwv(nd)/s2_sum
  END DO
  arg1(:) = s1*s2
  s12_dot = SUM(arg1(:))
