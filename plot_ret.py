...
def plot_var(var, dir='ret_code', axes=None, field_obs_on=True):
    ...
    field_laican = mlab.csv2rec('/home/users/if910917/projects/ret_tool_subprocess/state_files/mni_lai_canht_field_508_'
                         'mean.csv', comments='%')
    field_sm = mlab.csv2rec('/home/users/if910917/projects/ret_tool_subprocess/state_files/mni_sm_field_508_mean.csv',
                            comments='%')
    if axes is None:
        fig, ax = plt.subplots()
        ret_val = fig
...
def save_stats(var, dir):
    ...
    if var == 'sm':
        ...
        field_times = field_laican['_date'][:]
        field_ob = field_laican[var][:]
        t_idx = np.array([find_nearest(sat_times, dt.datetime.combine(x,dt.datetime.min.time()))[1]
