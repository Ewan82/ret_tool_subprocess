...
!     csq2_fw_fix
!
!> @brief Tapenade-compliant forward mode derivative of f(z) = |z|^2 with complex z
!
real(kind=8) function csq2_fw_fix(z, z_fw, csq2)
  implicit none
  ...
  csq2_fw_fix = 2*real(z)*real(z_fw) + 2*aimag(z)*aimag(z_fw)
  csq2 = real(z)**2 + aimag(z)**2
end function csq2_fw_fix


!***********************************************************
!     csq2_fw_fix
!
!> @brief Tapenade-compliant backward mode derivative of f(z) = |z|^2 with complex z
!
subroutine csq2_bw0_fix(z, z_bw, csq2_bw)
  implicit none
  complex(kind=8), intent(in) :: z
  complex(kind=8) :: z_bw
  real(kind=8) :: csq2_bw
