...
!
subroutine ncwrt_controlvector(fname, n, x, sx, succeed)
  use netcdf
  use mo_sensimul
  implicit none
  ! arguments
...
end subroutine ncwrt_controlvector


subroutine ncwrt_controlvector_old(fname, n, x, sx, xpr, sxpr, succeed)
  use netcdf
  use mo_sensimul
  implicit none
  ! arguments
  ...
  real(kind=8), intent(in)     :: x(n)
  real(kind=8), intent(in)     :: sx(n)
  real(kind=8), intent(in)     :: xpr(n)
  real(kind=8), intent(in)     :: sxpr(n)
  logical, intent(out)         :: succeed
  ! local decls
  integer, parameter :: nvar = 1+1+4*nsc !time,state-typ,statecomponents,their uncertainties,
                                         !prior statecomponents and their uncertainties
  real(kind=8), parameter :: state_fill_value = -99999.
  real(kind=8), allocatable :: statev(:,:), stateuncv(:,:)
  real(kind=8), allocatable :: prior_statev(:,:), prior_stateuncv(:,:)
  integer :: np
  integer :: rc, fid, dim_id
  integer :: dataset_ids(nvar) 
  character(len=16) :: dataset_name(nvar)
  character(len=16) :: dim_name
  integer :: i,ivar
  ...
  !-- do the same for prior
  allocate( prior_statev(nsc,npts), prior_stateuncv(nsc,npts) )
  prior_statev    = reshape( xpr(np+1:),  (/nsc,npts/) )
  prior_stateuncv = reshape( sxpr(np+1:), (/nsc,npts/) )

  !-- open file handle
  rc = nf90_create(trim(fname), NF90_NETCDF4, fid)
  if( rc.ne.nf90_noerr) then
     write(*,'(a,a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old: opening file '//&
          '***'//trim(fname)//'*** for writing failed: ',&
          trim(nf90_strerror(rc))
     return
  end if
  ...
  !-- create dimension
  dim_name = 'npoints'
  rc = nf90_def_dim(fid, dim_name, npts, dim_id)
  if( rc.ne.nf90_noerr) then
     write(*,'(a,a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old: creation of dimension '//&
          '---'//trim(dim_name)//'--- failed: ', trim(nf90_strerror(rc))
     return
  end if
  ...
  !-- time
  dataset_name(ivar) = 'time'
  rc = nf90_def_var(fid, dataset_name(ivar), NF90_REAL8, dim_id, dataset_ids(ivar), deflate_level=4)
  if( rc.ne.nf90_noerr ) then
     write(*,'(a,a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old:'//&
          'creation of dataset variable '//&
          '---'//trim(dataset_name(ivar))//'--- failed.', trim(nf90_strerror(rc))
     return
  endif
  ...
  dataset_name(ivar) = 'sim_typ'
  rc = nf90_def_var(fid, dataset_name(ivar), NF90_INT,&
       dim_id, dataset_ids(ivar), deflate_level=4)
  if( rc.ne.nf90_noerr ) then
     write(*,'(a,a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old:'//&
          'creation of dataset variable '//&
          '---'//trim(dataset_name(ivar))//'--- failed.', trim(nf90_strerror(rc))
  ...
  do i=1,nsc
     select case(i)
     case(1)
        dataset_name(ivar) = 'prior_lai'
     case(2)
        dataset_name(ivar) = 'prior_canht'
     case(3)
        dataset_name(ivar) = 'prior_sm'
     end select
     rc = nf90_def_var(fid, dataset_name(ivar), NF90_REAL8,&
          dim_id, dataset_ids(ivar), deflate_level=4)
     if( rc.ne.nf90_noerr ) then
  ...
  !-- prior state vector uncertainty
  do i=1,nsc
     select case(i)
     case(1)
        dataset_name(ivar) = 'prior_lai_unc'
     case(2)
        dataset_name(ivar) = 'prior_canht_unc'
     case(3)
        dataset_name(ivar) = 'prior_sm_unc'
     end select
     rc = nf90_def_var(fid, dataset_name(ivar), NF90_REAL8,&
          dim_id, dataset_ids(ivar), deflate_level=4)
     if( rc.ne.nf90_noerr ) then
  ...
  rc = nf90_put_var(fid, dataset_ids(ivar), timept_simtyp)
  if( rc.ne.nf90_noerr ) then
     write(*,'(a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old:'//&
          'error when writing data to file ---'//trim(dataset_name(ivar))//'---   '//&
          '(###'//trim(nf90_strerror(rc))//'###)'
     return
  endif
  ...
  do i=1,nsc
     call set_attributes(ivar)
     rc = nf90_put_var(fid, dataset_ids(ivar), prior_statev(i,:))
     if( rc.ne.nf90_noerr ) then
        write(*,'(a,a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old:'//&
             'error when writing data to file for ---'//trim(dataset_name(ivar))//'---  ', trim(nf90_strerror(rc))
        return
  ...
  do i=1,nsc
     call set_attributes(ivar)
     rc = nf90_put_var(fid, dataset_ids(ivar), prior_stateuncv(i,:))
     if( rc.ne.nf90_noerr ) then
        write(*,'(a,a)') ' ###NETCDF-ERROR::ncwrt_controlvector_old:'//&
             'error when writing data to file for ---'//trim(dataset_name(ivar))//'--- ', trim(nf90_strerror(rc))
        return
  ...
  contains
    subroutine set_attributes(ivar)
      ...
      select case( dataset_name(ivar) )
         ...
         case('prior_lai')
            call set_cfvar_attributes(fid, dataset_ids(ivar), dataset_name(ivar),&
                 'prior_LAI', 'Leaf area index (prior value)', 'm2/m2')
         case('prior_canht')
            call set_cfvar_attributes(fid, dataset_ids(ivar), dataset_name(ivar),&
                 'prior_canht', 'Canopy height (prior value)', 'm')
         case('prior_sm')
            call set_cfvar_attributes(fid, dataset_ids(ivar), dataset_name(ivar),&
                 'prior_SM', 'Soil moisture (prior value)', 'm3/m3')
         case('prior_lai_unc')
            call set_cfvar_attributes(fid, dataset_ids(ivar), dataset_name(ivar),&
                 'prior_LAI_unc', '1-sigma uncertainty of Leaf area index (prior value)', 'm2/m2')
         case('prior_canht_unc')
            call set_cfvar_attributes(fid, dataset_ids(ivar), dataset_name(ivar),&
                 'prior_canht_unc', '1-sigma uncertainty of Canopy height (prior value)', 'm')
         case('prior_sm_unc')
            call set_cfvar_attributes(fid, dataset_ids(ivar), dataset_name(ivar),&
                 'prior_SM_unc', '1-sigma uncertainty of Soil moisture (prior value)', 'm3/m3')
         case default
            write(*, '(a)') ' FATAL::set_attributes:unexpected dataset ---'//&
                 trim(dataset_name(ivar))//'---'
            stop
...
end subroutine ncwrt_controlvector_old
