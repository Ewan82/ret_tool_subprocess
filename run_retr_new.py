...
def run_ret_tool(out_dir, run_ret=True, **kwargs):
    ...
    ret_tool_dic = {'time_start': '20170323', 'time_end': '20170720', 'site_nml': dir_path+'/site.nml', 'states_file':
                    ...
                    'ctlvec_uncfloor': [1e-6, 1e-3, 1e-3, 1e-3], 's1_vv_uncfloor': 0.04, 's1_pol': ['VH'],
                    's2_bnds': ['2','3','4','5','6','7','8','8a'], 's1_vh_uncfloor':0.000001}
    if kwargs is not None:
        for key, value in kwargs.iteritems():
            if value is not False:
                ret_tool_dic[key] = value
    if run_ret is True:
        subprocess.call(['cp', '-R', dir_path+'/ret_code_new', out_dir])
        os.chdir(out_dir)
...
def run_retr_exp(exp_no, exp_tup, point='508_med', run_ret=True, field_obs=True):
    ...
    try:
        ...
        s2_str = 'nos2'
    save_dir = '/export/cloud/nceo/users/if910917/s3_exps/test2/'+point+'/'+prior+'_'+s1_str+'_'+s2_str
