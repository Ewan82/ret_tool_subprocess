...
def save_stats(var, dir, site='ita', point='sg01'):
    ...
    sat_times = nc.num2date(post.variables['time'][:], post.variables['time'].units)
    if var == 'sm':
        t_idx = np.array([find_nearest_idx_tol(field_sm['date'], x, tol=dt.timedelta(seconds=60*60*6))
                          for x in sat_times])
        ...
        ret_t_idx = np.array([find_nearest_idx_tol(sat_times, x, tol=dt.timedelta(seconds=60*60*6))
...
def plot_refl_mod(_dir, point, axes=None):  # write a fn that plots obs and modelled refl for given point!
    ...
    sat_obs = mlab.csv2rec('/home/users/if910917/projects/ret_tool_subprocess/S2/ita_s2_'+
...
def plot_backscat(_dir, point, axes=None):  # write a fn that plots obs and modelled refl for given point!
    if axes is None:
        ...
        ax = axes
        ret_val = ax
    sat_obs = mlab.csv2rec('/home/users/if910917/projects/ret_tool_subprocess/S1/ita_s1_'+
